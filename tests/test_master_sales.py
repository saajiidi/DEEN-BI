...
class TestMasterSales(unittest.TestCase):
    def test_load_master_sales_dataset_uses_parquet_snapshot_when_workbook_missing(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch.object(master_sales, "CORE_PARQUET_SNAPSHOT_PATH", snapshot_path),
                patch.object(master_sales, "MASTER_CACHE_FILE", cache_path),
                patch.object(master_sales, "CORE_WORKBOOK_PATH", workbook_path),
                patch.object(
                    master_sales,
                    "_load_2026_delta",
                    lambda base_df, force_refresh=False: pd.DataFrame(columns=base_df.columns),
                ),
            ):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                patch.object(
                    master_sales,
                    "_load_core_workbook",
                    lambda: (workbook_df, {"base_rows": 1, "sheet_count": 3}),
                ),
                patch.object(
                    master_sales,
                    "_load_2026_delta",
