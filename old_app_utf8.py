...
# --- Tabs ---
t_dash, t_order, t_inv, t_wp, t_logs = st.tabs(["≡ƒôè Executive Dashboard", "≡ƒôª Pathao Processor", "≡ƒÅó Distribution Matrix", "≡ƒÆ¼ WP Verification", "≡ƒ¢á∩╕Å System Logs"])

# ---------------------------------------------------------
# TAB 0: EXECUTIVE DASHBOARD
# ---------------------------------------------------------
with t_dash:
    st.markdown("### ≡ƒôê Business Performance Insights")
    ...
    if st.session_state.get('inv_res_data') is not None:
        ...
        with c1:
            st.markdown("#### ≡ƒÅÑ Stock Health Heatmap")
            # Melt for heatmap
        ...
        with c2:
            st.markdown("#### ≡ƒôë Distribution Balance")
        ...
        # Low Stock Alerts
        st.markdown("#### ΓÜá∩╕Å Immediate Replenishment Required")
        ...
        if not low_stock_items.empty:
            ...
            st.success("All stock levels are above the safety threshold. Excellent! Γ£à")
    else:
        st.info("≡ƒÆí Run a distribution analysis in the 'Distribution Matrix' tab to populate this dashboard.")

# ---------------------------------------------------------
# TAB 1: PATHAO ORDER PROCESSOR (With Auto-Repair)
# ---------------------------------------------------------
with t_order:
    st.markdown("### Γ£¿ Pathao Intelligence & Address Repair")
    up_pathao = st.file_uploader("≡ƒôé Drop Orders File", type=['xlsx', 'csv'], key="pathao_up")
    
    if up_pathao:
        try:
            with st.spinner("≡ƒÜÇ Analyzing for Address Integrity..."):
            ...
            # Address Repair UI
            if any(invalid_mask):
                st.markdown(f"#### ≡ƒ¢á∩╕Å Found {sum(invalid_mask)} Potential Address Issues")
            ...
            st.download_button("≡ƒôÑ Download Repaired Pathao Data", buf.getvalue(), "Pathao_Final.xlsx")
...
# ---------------------------------------------------------
with t_inv:
    st.markdown("### ≡ƒÅó Active Stock Matrix & Thresholds")
    
    c_m, c_p = st.columns([3, 1])
    with c_p:
        st.markdown("#### ΓÜÖ∩╕Å Thresholds")
        st.session_state.low_stock_threshold = st.number_input("Safety Stock Level", value=st.session_state.get('low_stock_threshold', 5))
        search_q = st.text_input("≡ƒöì Search SKU / Name", key="inv_matrix_search")
        if st.button("Save State ≡ƒÆ╛"): save_state()
    ...
    if m_file and st.button("≡ƒÜÇ Analyze Distribution"):
        try:
    ...
    if st.session_state.get('inv_res_data') is not None:
        ...
        st.download_button("≡ƒôÑ Download Distribution Report", buf_inv.getvalue(), "Stock_Distribution.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

# ---------------------------------------------------------
# TAB 3: WP VERIFICATION (With Bulk Export)
# ---------------------------------------------------------
with t_wp:
    st.markdown("### ≡ƒÆ¼ Verification Center")
    wp_f = st.file_uploader("≡ƒôé Verification List", key="wp_up_2")
    ...
    if wp_f:
        try:
            ...
            with c2:
                ...
                st.download_button("≡ƒôÑ Export Bulk Message Text", bulk_text, "Bulk_WhatsApp_Messages.txt")
...
# ---------------------------------------------------------
with t_logs:
    st.markdown("### ≡ƒ¢á∩╕Å Developer Control")
    logs = get_logs()
    if logs:
