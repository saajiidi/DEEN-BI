...
def normalize_sales_dataframe(
    df: pd.DataFrame, source_tab: str = "", column_mapping: Optional[dict] = None
    ...
    if df is None or df.empty:
        ...
        return empty_df, NormalizationResult(
            df=empty_df, column_mapping={}, unmapped_columns=[], row_count=0
        )
    ...
    if "unit_price" in normalized.columns and "qty" in normalized.columns:
        normalized["unit_price"] = pd.to_numeric(normalized["unit_price"], errors="coerce").fillna(
            0
        )
...
def compute_sales_analytics(df: pd.DataFrame) -> dict:
    ...
    if "category" in df.columns and "line_amount" in df.columns:
        summary = df.groupby("category").agg({"qty": "sum", "line_amount": "sum"}).reset_index()
    ...
    if "order_key" in df.columns:
        order_level = df.groupby("order_key").agg({"qty": "sum", "line_amount": "sum"})
        basket = {
    ...
    if "order_date" in df.columns and df["order_date"].notna().any():
        ...
        trends = (
            ...
            .agg({"line_amount": "sum", "order_key": "nunique"})
            .reset_index()
        )
    ...
    if "item_name" in df.columns and df["item_name"].notna().any():
        top_products = (
            ...
            .agg({"line_amount": "sum", "qty": "sum", "order_key": "nunique"})
            .reset_index()
        )
...
def filter_by_date_range(
    df: pd.DataFrame, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None
) -> pd.DataFrame:
...
def compute_period_over_period(
    ...
    # Calculate deltas
    current_rev = (
        current_analytics["summary"]["Total Amount"].sum()
        if not current_analytics["summary"].empty
        else 0
    )
    previous_rev = (
        previous_analytics["summary"]["Total Amount"].sum()
        if not previous_analytics["summary"].empty
        else 0
    )

    current_qty = (
        current_analytics["summary"]["Total Qty"].sum()
        if not current_analytics["summary"].empty
        else 0
    )
    previous_qty = (
        previous_analytics["summary"]["Total Qty"].sum()
        if not previous_analytics["summary"].empty
        else 0
    )
