...
def render_executive_summary(df_sales: pd.DataFrame, df_customers: pd.DataFrame, summary: dict):
    ...
    with k1: st.metric("Revenue", f"TK {total_revenue:,.0f}"); ui.badge("Order-level totals")
    with k2: st.metric("Orders", f"{total_orders:,}"); ui.badge("Normalized count")
    with k3: st.metric("AOV", f"TK { (total_revenue/total_orders) if total_orders else 0:,.0f}"); ui.badge("Direct calc")
    with k4: st.metric("Customers", f"{active_customers:,}"); ui.badge("Distinct keys")
    with k5: st.metric("Pending", f"{pending_count:,}", "Action required" if pending_count > 5 else "Healthy")

    s1, s2, s3 = st.columns(3)
    with s1: st.metric("Items Sold", f"{total_items:,.0f}"); st.caption(f"WooCommerce: {summary.get('woocommerce_live',0):,} rows")
    with s2: st.metric("Repeat Rate", f"{float((df_customers['total_orders'] > 1).mean() * 100) if not df_customers.empty else 0:.1f}%"); ui.badge("Retention base")
    with s3: st.metric("Latest Order", df["order_date"].max().strftime("%Y-%m-%d %H:%M") if not df.empty and pd.notna(df["order_date"].max()) else "N/A")
